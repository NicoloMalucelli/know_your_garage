...
export default defineComponent({
  ...
  methods: {
    ...
    changePassword(){
      ...
      if(this.admin()){
        axios.patch('http://localhost:3000/admins/' + sessionStorage.getItem("email"), {"password": sha256(this.new_pw).toString()}).then((response) =>{
  ...
  mounted() {
    if(this.admin()){
      axios.get('http://localhost:3000/admins/' + sessionStorage.getItem("email")).then(response => {
        this.user.name = response.data.company_name
